...
class StripeService
  def self.new_stripe_customer(buyer)
  ...
  def self.charge_customer(final_amount, stripe_customer_source, customer_id)
  ...
  def self.update_stripe_customer(stripe_cust_id, buyer)
  ...
  def self.destroy_stripe_customer(buyer)
  ...
  def self.create_stripe_plan(monthly_fee, name)
    Stripe::Price.create({
